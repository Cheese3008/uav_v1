...
struct HsvRange
{
    ...
    // Neu min[0] > max[0], detector se tu tach hue thanh 2 khoang de ho tro mau do bi wrap.
...
struct point
{
    ...
    // Toa do tuong doi trong he BODY-FRD tai thoi diem kich mode.
    ...
    // Nguong HSV min/max dai dien cho vong tron/khoi tai diem nay.
    HsvRange hsvRange{};

    // Cho phep can chinh bang xu ly anh sau khi bay toi toa do local.
    bool useImageServo{true};

    // Neu true, node se publish thong bao drop sau khi da can giua anh va giu on dinh.
...
{
    // Giu lai nhom tham so cu de quan ly timeout/projection.
    float stableHoldSec{1.0f};
    float dropDelaySec{1.0f};
