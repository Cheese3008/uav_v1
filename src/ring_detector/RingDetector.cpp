	...
	RingCandidate &best) const
{
	...
	for (const auto &c : candidates)
	{
		...
		const double size_ratio = static_cast<double>(c.visible_size) / std::max(1.0f, _lock_visible_size);
...
bool RingDetectorNode::confirm_far_candidate(const RingCandidate &candidate)
{
	...
	const double size_ratio_err = std::abs(static_cast<double>(candidate.visible_size) - size_ref) / size_ref;
