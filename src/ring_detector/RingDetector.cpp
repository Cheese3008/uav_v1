...
// constexpr double kNearHoughParam2 = 26.0;
...
// constexpr double kFarHoughParam2 = 20.0;
...
// 	const double radius_std_limit = std::max(kCircleMaxRadiusStd, radius * 0.035);
// 	const double radius_std_score = 1.0 - std::min(
// 		1.0,
...
// 	cv::GaussianBlur(clahe_img, blur, cv::Size(5, 5), 1.2);
// 	cv::Canny(blur, edges, 80, 160);

// 	const int max_radius = std::max(
// 		static_cast<int>(std::min(frame.cols, frame.rows) * 0.45),
...
// 	const int max_radius = std::max(
// 		static_cast<int>(std::min(frame.cols, frame.rows) * 0.28),
// 		min_radius + 2);
...
// 		const double score =
// 			-1.8 * dist_center +
// 			0.55 * static_cast<double>(c.visible_size) +
// 			120.0 * support_ratio -
// 			15.0 * radius_std -
// 			10.0 * static_cast<double>(max_gap) +
// 			100.0 * confidence;
